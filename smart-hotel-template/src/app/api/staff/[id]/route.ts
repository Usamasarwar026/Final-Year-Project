...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const summary = {
      ...
      totalHours: user.attendance.reduce((s, a) => s + (Number(a.hours) || 0), 0),
    };

    return NextResponse.json({
      staff: { ...user, password: undefined, staffProfile: user.staffProfile ? { ...user.staffProfile, basic_salary: user.staffProfile.basic_salary ? Number(user.staffProfile.basic_salary) : null } : null },
...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const {
      ...
      joining_date, basic_salary, performance_notes, is_on_duty,
    } = body;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (department_id     !== undefined) staffUpdate.department_id     = department_id;
      if (designation       !== undefined) staffUpdate.designation       = designation;
      if (shift_id          !== undefined) staffUpdate.shift_id          = shift_id;
      if (joining_date      !== undefined) staffUpdate.joining_date      = joining_date ? new Date(joining_date) : null;
      if (basic_salary      !== undefined) staffUpdate.basic_salary      = basic_salary;
      if (performance_notes !== undefined) staffUpdate.performance_notes = performance_notes;
    ...
    try {
      ...
      await createNotification({
        ...
        message: "Your profile details have been updated by Admin.",
        type: "staff", priority: "Low", module: "staff",
        reference_id: id, recipient_user_id: id,
      });
    } catch (notifErr) {
    ...
    return NextResponse.json({
      staff: {
        ...
        staffProfile: updated?.staffProfile
          ? { ...updated.staffProfile, basic_salary: updated.staffProfile.basic_salary ? Number(updated.staffProfile.basic_salary) : null }
          : null,
      },
    });
  } catch (err) {
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.staff.updateMany({ where: { user_id: id }, data: { is_active: false, is_on_duty: false } }),
    ]);
    ...
    try {
      ...
      await createNotification({
        ...
        type: "staff", priority: "High", module: "staff",
        reference_id: id, recipient_user_id: id,
      });
    } catch (notifErr) {
