...
import { getConsumptionReport } from "@/services/inventoryService";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const report = await getConsumptionReport(from, to);
    return NextResponse.json({ report });
  } catch (err) {
    console.error("[GET /api/inventory/reports/consumption]", err);
