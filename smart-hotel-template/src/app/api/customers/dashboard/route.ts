...
export async function GET() {
  try {
    ...
    // 7. Pending laundry orders (if active booking)
    const pendingLaundry = activeBooking
      ? await prisma.laundryRecord.count({
          where: { booking_id: activeBooking.booking_id, status: "Pending" },
        })
      : 0;
