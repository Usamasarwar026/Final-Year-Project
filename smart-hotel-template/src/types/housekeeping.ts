...
// ─── Enums ────────────────────────────────────────────────────────────────────
export type TaskType     = "Cleaning" | "LaundryPickup" | "ServiceRequest" | "Maintenance";
...
export type RequestType  = "Towels" | "Laundry" | "RoomService" | "Water" | "Other";
...
export const TASK_TYPE_CONFIG: Record<TaskType, { label: string; icon: string; color: string; bg: string }> = {
  Cleaning:       { label: "Cleaning",        icon: "🧹", color: "text-blue-700",   bg: "bg-blue-100"   },
  LaundryPickup:  { label: "Laundry Pickup",  icon: "👕", color: "text-purple-700", bg: "bg-purple-100" },
  ServiceRequest: { label: "Service Request", icon: "🔔", color: "text-amber-700",  bg: "bg-amber-100"  },
  Maintenance:    { label: "Maintenance",     icon: "🔧", color: "text-red-700",    bg: "bg-red-100"    },
};
...
export const TASK_STATUS_CONFIG: Record<TaskStatus, { label: string; color: string; bg: string; border: string; dot: string }> = {
  ...
  Done:       { label: "Done",        color: "text-green-700",  bg: "bg-green-50",  border: "border-green-200",  dot: "bg-green-500"  },
  Cancelled:  { label: "Cancelled",   color: "text-red-700",    bg: "bg-red-50",    border: "border-red-200",    dot: "bg-red-400"    },
};

export const CLEANING_STATUS_CONFIG: Record<CleaningStatus, { label: string; color: string; bg: string; dot: string }> = {
  Clean:      { label: "Clean",       color: "text-green-700", bg: "bg-green-100", dot: "bg-green-500" },
  Dirty:      { label: "Dirty",       color: "text-red-700",   bg: "bg-red-100",   dot: "bg-red-500"   },
  InProgress: { label: "In Progress", color: "text-blue-700",  bg: "bg-blue-100",  dot: "bg-blue-500"  },
};
...
export const REQUEST_STATUS_CONFIG: Record<RequestStatus, { label: string; color: string; bg: string; dot: string }> = {
...
export const LAUNDRY_STATUS_CONFIG: Record<LaundryStatus, { label: string; color: string; bg: string }> = {
  Pending:  { label: "Pending",  color: "text-amber-700", bg: "bg-amber-50"  },
  Sent:     { label: "Sent",     color: "text-blue-700",  bg: "bg-blue-50"   },
  Returned: { label: "Returned", color: "text-green-700", bg: "bg-green-50"  },
};

export const TASK_TYPES:    TaskType[]    = ["Cleaning", "LaundryPickup", "ServiceRequest", "Maintenance"];
export const TASK_PRIORITIES: TaskPriority[] = ["Normal", "High", "VIP"];
export const TASK_STATUSES: TaskStatus[]  = ["Pending", "InProgress", "Done", "Cancelled"];
export const REQUEST_TYPES: RequestType[] = ["Towels", "Laundry", "RoomService", "Water", "Other"];
export const LAUNDRY_STATUSES: LaundryStatus[] = ["Pending", "Sent", "Returned"];
...
export interface HousekeepingTask {
  ...
  updated_at:          string;
  // Relations
  room?:          { room_number: string; floor: number; room_type: string; cleaning_status: CleaningStatus } | null;
  assignedStaff?: { staff_id: number; designation: string; user: { name: string; email: string } } | null;
  booking?:       { booking_id: number; check_in_date: string; check_out_date: string } | null;
}
