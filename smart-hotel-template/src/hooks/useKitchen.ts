...
export function useKitchenOrders(filters?: {
  ...
  return useQuery({
    ...
    refetchInterval: 10 * 1000, // Poll every 10s for live KDS updates
  });
}
...
export function useStaffByUserId(userId: string) {
  return useQuery({
    ...
    queryFn: () => kitchenService.getStaffByUserId(userId),
    enabled: !!userId,
    staleTime: 5 * 60 * 1000,
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.payload.assigned_to) {
        ...
        toast.success(`Status updated → ${variables.payload.status}`);
      }
    },
    onError: (err: any) => {
      toast.error(
