...
export const kitchenService = {
  ...
  getDeliveryStaff: async () => {
    const { data } = await api.get<{ staff: DeliveryStaff[] }>("/kitchen/delivery-staff");
    return data.staff;
  },

  getStaffByUserId: async (userId: string) => {
    const { data } = await api.get<{ staff: KitchenStaff }>(`/kitchen/staff-by-user/${userId}`);
    return data.staff;
  },

  createKitchenStaff: async (payload: {
    userId: string;
    designation: string;
    department: string;
  }) => {
    // Uses the shared POST /api/staff endpoint
    const { data } = await api.post("/staff", payload);
    return data.staff;
  },

  updateKitchenStaff: async (
    id: number,
    payload: { designation?: string; is_active?: boolean; is_on_duty?: boolean }
  ) => {
    // Uses the shared PATCH /api/staff/[userId] endpoint
