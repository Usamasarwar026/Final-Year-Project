...
import {
  CalendarCheck, Users, Clock, CheckCircle2, Search, X, Eye,
  Trash2, BedDouble, Plus, ChevronDown, UserPlus, Mail, Phone,
  MapPin, CreditCard, ArrowLeft, Loader2, Info, UserCheck,
...
// ─── Pagination ───────────────────────────────────────────────────────────────
function Pagination({ page, totalPages, total, limit, isFetching, onPageChange, onLimitChange }: {
...
function BookingDetailModal({ booking, onClose, onStatusChange, updatingId, updatingStatus }: {
  booking: Booking;
  onClose: () => void;
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {booking.source === "ADMIN" ? "Admin booking" : "Customer booking"} · {fmt(booking.created_at)}
              </p>
            </div>
        ...
        <div className="flex gap-2.5 px-5 py-4 border-t border-border shrink-0">
          ...
          {actions.map((a) => {
            ...
            return (
              ...
              >
                {isThisAction ? <Loader2 size={14} className="animate-spin mx-auto" /> : a.label}
              </button>
            );
          })}
        </div>
              ...
              {loading ? (
                ...
                filtered.map((c) => (
                  <button
                    ...
                    className={clsx("w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-muted/60 transition-colors", selected?.id === c.id && "bg-accent/8")}
                  >
...
function RoomDropdown({ rooms, selected, onSelect, loading, error, disabled }: {
  rooms: Room[]; selected: Room | null; onSelect: (r: Room) => void;
  ...
  useEffect(() => {
    if (selected && !rooms.find((r) => r.room_id === selected.room_id)) onSelect(null as any);
  }, [rooms]);
                ...
                {rooms.map((room) => {
                  ...
                  return (
                    <button
                      ...
                      className={clsx("w-full flex items-start gap-3 px-3.5 py-3 text-left hover:bg-muted/60 border-b border-border/50 last:border-0 transition-colors", selected?.room_id === room.room_id && "bg-accent/8")}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {room.bed_type} Bed{room.size_sqft ? ` · ${room.size_sqft} sqft` : ""}
                        </p>
                        {amenities.length > 0 && (
...
function CreateCustomerPanel({ onBack, onCreated, createCustomer }: {
  ...
  const handleCreate = async () => {
    ...
    setSaving(false);
    if (res.ok && res.data) {
      onCreated(res.data, `hotel@${new Date().getFullYear()}`);
    } else {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
      <div className="flex items-center gap-2">
        <button type="button" onClick={onBack} className="p-1.5 rounded-xl hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
          <ArrowLeft size={15} />
        </button>
        <div>
      ...
      <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl">
        ...
        <p className="text-[11px] text-blue-700">A temporary password will be generated and sent to the customer's email.</p>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onDismiss} className="p-1 text-emerald-600 hover:text-emerald-800"><X size={13} /></button>
      </div>
      <div className="bg-white/70 rounded-lg p-2.5 space-y-1 text-xs">
        ...
        <p className="text-emerald-600 text-[10px]">📧 Credentials sent to customer's email.</p>
      </div>
    </motion.div>
  );
}
...
function CreateBookingModal({ onClose, onCreated }: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Fixed: was referencing undefined `error` variable
      ...
      >
                  ...
                  {selectedCust && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden">
                      <div className="flex gap-3 flex-wrap text-[11px] text-muted-foreground mt-1 pl-1">
                        {selectedCust.phoneNumber && (
                          <span className="flex items-center gap-1"><Phone size={10} />{selectedCust.phoneNumber}</span>
                        )}
                        {selectedCust.city && (
                          <span className="flex items-center gap-1">
                            <MapPin size={10} />{selectedCust.city}{selectedCust.country ? `, ${selectedCust.country}` : ""}
                          </span>
                        )}
                        <span className="flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => { setCheckIn(e.target.value); if (e.target.value >= checkOut) setCheckOut(addDays(e.target.value, 1)); setSelectedRoom(null); }}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {!roomsLoading && datesValid && rooms.length > 0 && (
                      <span className="text-[10px] text-muted-foreground">{rooms.length} available</span>
                    )}
                  </div>
                  <RoomDropdown
                    rooms={rooms} selected={selectedRoom} onSelect={setSelectedRoom}
                    loading={roomsLoading} error={roomsError} disabled={!datesValid}
                  />
        ...
        {step === "main" && (
          <div className="flex gap-3 px-5 py-4 border-t border-border shrink-0">
            ...
            >
              {saving ? <><Loader2 size={14} className="animate-spin" /> Creating…</> : <><CalendarCheck size={14} /> Confirm Booking</>}
            </button>
          </div>
        )}
      </motion.div>
...
export default function AdminBookings() {
  // ✅ All useState BEFORE useBookings — no hoisting issues
  ...
  const {
    bookings, pagination, loading, isFetching, error, refresh,
    updateStatus, deleteBooking, updatingId, updatingStatus, isDeleting, deletingId,
  } = useBookings({ page, limit, search: debouncedSearch, status: filterStatus });

  const handleSearch = (val: string) => { setSearch(val); setPage(1); };
  const handleStatusFilter = (val: BookingStatus | "All") => { setFilterStatus(val); setPage(1); };
  ...
  const handleDelete = (booking: Booking) => setDeleteTarget(booking);

  const confirmDelete = async () => {
    if (!deleteTarget) return;
    const res = await deleteBooking(deleteTarget.booking_id);
    if (res.ok) { toast.success("Booking deleted"); setDeleteTarget(null); }
    else toast.error(res.error ?? "Failed to delete");
  };

  // Stats from current page (server handles filtering)
  const stats = {
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage all reservations, check-ins and check-outs</p>
        </div>
        <button
      ...
      <div className="bg-background border border-border rounded-2xl overflow-hidden">
                  ...
                  bookings.map((b, i) => {
                    ...
                    return (
                      <motion.tr
                        ...
                        className={clsx("border-t border-border hover:bg-muted/30 transition-colors", (isRowUpdating || isRowDeleting) && "opacity-60")}
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <span className={clsx("text-[9px] font-semibold px-1.5 py-0.5 rounded", b.source === "ADMIN" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700")}>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1">
                            ...
                            >
                              {isRowDeleting ? <Loader2 size={13} className="animate-spin text-red-400" /> : <Trash2 size={13} />}
                            </button>
                          </div>
                        </td>
                      </motion.tr>
        ...
        {!loading && !error && pagination && (
          <Pagination
            page={page} totalPages={pagination.totalPages} total={pagination.total}
            limit={limit} isFetching={isFetching}
      ...
      <AnimatePresence>
        {viewBooking && (
          <BookingDetailModal
            booking={viewBooking} onClose={() => setViewBooking(null)}
            onStatusChange={handleStatusChange}
            updatingId={updatingId} updatingStatus={updatingStatus}
          />
        )}
      </AnimatePresence>
