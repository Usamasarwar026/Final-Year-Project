...
import {
  ...
  List, Phone, MapPin, Star, UserPlus, Send, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const STATUS_TRANSITIONS: Record<FoodOrderStatus, { label: string; next: FoodOrderStatus; color: string; requiresStaff?: boolean }[]> = {
  ...
  Ready:          [{ label: "Assign",    next: "Assigned",       color: "bg-purple-600 text-white hover:bg-purple-700", requiresStaff: true }],
  Assigned:       [{ label: "Out for Delivery", next: "OutForDelivery", color: "bg-orange-600 text-white hover:bg-orange-700" }],
        ...
        <tr key={i} className="border-t border-border">
          {Array.from({ length: 9 }).map((_, j) => (
            <td key={j} className="px-4 py-3.5">
              <div className={clsx("h-4 bg-muted/60 rounded animate-pulse", j === 0 ? "w-10" : j === 2 ? "w-24" : j === 8 ? "w-20" : "w-16")} />
            </td>
          ))}
        </tr>
      ))}
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/20">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-foreground">Assign Delivery Staff</h2>
          </div>
          ...
          <div className="max-h-64 overflow-y-auto space-y-2">
            {filteredStaff.length === 0 ? (
              <div className="py-8 text-center">
                <UserPlus size={32} className="mx-auto text-muted-foreground/30 mb-2" />
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {assignedStaff && (
            <div className="mx-5 mt-4 p-3 rounded-xl bg-purple-50 border border-purple-200">
              <p className="text-[10px] font-semibold text-purple-700 uppercase tracking-wide">Assigned Delivery Staff</p>
              <div className="flex items-center gap-3 mt-2">
                ...
                {assignedStaff.user?.phoneNumber && (
                  <a href={`tel:${assignedStaff.user.phoneNumber}`} className="ml-auto p-2 rounded-lg bg-purple-100 text-purple-700">
                    <Phone size={14} />
                  </a>
                )}
              </div>
          ...
          {actions.map((a) => {
            ...
            return (
              <button
                ...
                disabled={isLoading}
                className={clsx("flex-1 px-4 py-2.5 rounded-xl text-xs font-bold transition-opacity hover:opacity-90 flex items-center justify-center gap-1.5 disabled:opacity-60", a.color)}
              >
...
// ─── KDS Card (Kanban) ────────────────────────────────────────────────────────
function KDSCard({ order, onView, onStatusChange, onAssignClick, loadingOrderId }: {
  order: FoodOrder;
  ...
  return (
    ...
    >
      ...
      {/* Assigned Staff Info (for Assigned/OutForDelivery status) */}
      {assignedStaff && (order.status === "Assigned" || order.status === "OutForDelivery") && (
        <div className="flex items-center gap-2 p-2 rounded-lg bg-purple-50">
          ...
          <div className="flex-1">
            <p className="text-[10px] font-semibold text-foreground">{assignedStaff.user?.name}</p>
          </div>
          {assignedStaff.user?.phoneNumber && (
            <a href={`tel:${assignedStaff.user.phoneNumber}`} className="text-purple-600">
              <Phone size={10} />
            </a>
          )}
        </div>
        ...
        {actions.map((a) => {
          if (a.requiresStaff && onAssignClick) {
            return (
              <button
                ...
                onClick={() => onAssignClick(order)}
                disabled={isLoading}
                className="flex-1 py-1.5 rounded-xl text-[10px] font-bold bg-purple-600 text-white hover:bg-purple-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
              >
                {isLoading ? <Loader2 size={10} className="animate-spin" /> : <Bike size={10} />}
                {a.label}
              </button>
            );
          }
          return (
            <button
              ...
              onClick={() => onStatusChange(order.id, a.next)}
              disabled={isLoading}
              className={clsx("flex-1 py-1.5 rounded-xl text-[10px] font-bold transition-opacity hover:opacity-90 flex items-center justify-center gap-1 disabled:opacity-60", a.color)}
            >
...
export default function KitchenOrders() {
  ...
  const kdsColumns: { key: FoodOrderStatus; label: string; color: string }[] = [
    ...
    { key: "OutForDelivery", label: "Out for Delivery", color: "border-t-orange-500" },
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {(search || statusFilter || typeFilter) && (
          <button onClick={() => handleFilterChange(() => { setSearch(""); setStatusFilter(""); setTypeFilter(""); })}
        ...
        isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 overflow-x-auto pb-4">
            {kdsColumns.map(({ key, label, color }) => {
              ...
              return (
                <div key={key} className={clsx("bg-muted/30 border-t-4 border border-border rounded-2xl p-3 space-y-3 min-w-[280px]", color)}>
                    ...
                    <div className="space-y-3 max-h-[calc(100vh-300px)] overflow-y-auto">
                      {colOrders.map((order) => (
                        <KDSCard
                          ...
                          onAssignClick={order.status === "Ready" ? setOrderToAssign : undefined}
                  ...
                  paginatedOrders.map((order, i) => {
                    ...
                    return (
                      <motion.tr key={order.id}
                        ...
                        className={clsx("border-t border-border transition-colors", isRowLoading ? "bg-primary/3 opacity-75" : "hover:bg-muted/20")}
                      >
                            ...
                            {actions.map((a) => {
                              if (a.requiresStaff) {
                                return (
                                  ...
                                  >
                                    {isRowLoading ? <Loader2 size={10} className="animate-spin" /> : <Bike size={10} />}
                                    {a.label}
                                  </button>
                                );
                              }
                              return (
                                <button
                                  ...
                                  disabled={isRowLoading}
                                  className={clsx("px-2 py-1.5 rounded-lg text-[10px] font-bold transition-opacity hover:opacity-80 flex items-center gap-1 disabled:opacity-50", a.color)}
                                >
          ...
          {!isLoading && filteredOrders.length > 0 && (
            <div className="px-4 py-3 border-t border-border bg-muted/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <span className="text-xs text-muted-foreground">
                Showing {Math.min((page - 1) * PAGE_SIZE + 1, filteredOrders.length)}–{Math.min(page * PAGE_SIZE, filteredOrders.length)} of {filteredOrders.length} orders
                &nbsp;·&nbsp;
