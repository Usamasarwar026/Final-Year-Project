...
//   const [tab, setTab] = useState<"departments" | "shifts">("departments");
...
//             tab === "departments" ? "bg-primary text-white" : "bg-muted"
//           }`}
//         >
//           Departments
//         </button>
...
//           className={`px-4 py-2 rounded-xl text-sm font-medium ${
//             tab === "shifts" ? "bg-primary text-white" : "bg-muted"
//           }`}
//         >
//           Shifts
//         </button>
...
//       {tab === "departments" ? <DepartmentSection /> : <ShiftSection />}
//     </div>
//   );
// }

// function DepartmentSection() {
//   const { departments, createDepartment, deleteDepartment, updateDepartment } =
//     useDepartments();
...
//           className="h-10 px-4 rounded-xl bg-primary text-white flex items-center gap-2"
//         >
//           <Plus size={16} />
//           Add Department
//         </button>
...
//                       setEditName("");
//                     }}
//                     className="px-3 py-1 text-xs rounded-lg bg-muted"
//                   >
//                     Cancel
//                   </button>
//                 </>
...
import {
  Building2, Clock, Plus, Pencil, Trash2,
  X, Check, Loader2, AlertCircle, ChevronRight,
} from "lucide-react";
...
function DeptForm({
  ...
  onSave:   (data: { name: string; icon: string; color: string; bg: string }) => Promise<void>;
  onCancel: () => void;
  saving:   boolean;
}) {
  ...
  return (
    <div className="space-y-4 p-4 bg-muted/30 rounded-2xl border border-border">
      ...
      {/* Color picker */}
      <div>
        <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Badge Color</label>
        <div className="flex flex-wrap gap-2">
          {COLOR_OPTIONS.map((c) => (
            <button
              key={c.label}
              ...
              className={clsx(
                ...
                c.bg, c.color,
                color.label === c.label ? "border-primary scale-105" : "border-transparent"
              )}
            >
              {icon} {c.label}
            </button>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={clsx("inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full", color.bg, color.color)}>
...
function ShiftForm({
  ...
  onSave:   (data: { name: string; start_time: string; end_time: string }) => Promise<void>;
  onCancel: () => void;
  saving:   boolean;
}) {
  const [name,  setName]  = useState(initial?.name       ?? "");
  ...
  const handleSave = async () => {
    ...
    if (!start)       { toast.error("Start time is required"); return; }
    if (!end)         { toast.error("End time is required");   return; }
  ...
  return (
    <div className="space-y-4 p-4 bg-muted/30 rounded-2xl border border-border">
      ...
      <div className="flex items-center gap-3 p-2.5 bg-background rounded-xl border border-border">
        ...
        <div>
          <p className="text-xs font-semibold text-foreground">{name || "Shift Name"}</p>
          <p className="text-[10px] text-muted-foreground">{start} – {end} · {duration}</p>
        </div>
      </div>
    ...
    <div className="bg-background border border-border rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-muted/30">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-muted-foreground">{count} configured</p>
        </div>
      </div>
...
export default function SettingStaffTab() {
  const { departments, loading: deptLoading, createDepartment, updateDepartment, deleteDepartment, refresh: refreshDepts } = useDepartments();
  const { shifts, loading: shiftLoading,     createShift,     updateShift,     deleteShift,     refresh: refreshShifts }     = useShifts();
  ...
  // ── Dept handlers ──────────────────────────────────────────────────────────
  const handleSaveDept = async (payload: { name: string; icon: string; color: string; bg: string }) => {
    setSaving(true);
    let res;
    if (deptForm === "add") {
    ...
    setSaving(false);
    if (res.ok) { toast.success(deptForm === "add" ? "Department created" : "Department updated"); setDeptForm(null); }
    else toast.error(res.error ?? "Failed");
  };

  const handleDeleteDept = async (d: DepartmentConfig) => {
    if (!confirm(`Delete department "${d.name}"? Staff assigned to it will lose their department.`)) return;
    setDeleting(d.id);
  ...
  // ── Shift handlers ─────────────────────────────────────────────────────────
  const handleSaveShift = async (payload: { name: string; start_time: string; end_time: string }) => {
    setSaving(true);
    let res;
    if (shiftForm === "add") {
    ...
    setSaving(false);
    if (res.ok) { toast.success(shiftForm === "add" ? "Shift created" : "Shift updated"); setShiftForm(null); }
    else toast.error(res.error ?? "Failed");
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      {/* ── DEPARTMENTS ────────────────────────────────────────────── */}
      <SectionCard title="Departments" icon={Building2} count={departments.length}>
            ...
            {departments.map((d) => (
              <div key={d.id}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-border hover:bg-muted/30 transition-colors group">
                  {/* Badge preview */}
                  <span className={clsx("inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full shrink-0", d.bg, d.color)}>
                    {d.icon} {d.name}
                  </span>
                  <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => setDeptForm(deptForm === d.id ? null : d.id)}
              ...
              <div key={s.id}>
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-border hover:bg-muted/30 transition-colors group">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => setShiftForm(shiftForm === s.id ? null : s.id)}
                      className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
                    >
                      <Pencil size={12} />
                    </button>
