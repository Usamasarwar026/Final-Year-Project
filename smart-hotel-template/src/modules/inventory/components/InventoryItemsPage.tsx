...
export default function InventoryItemsPage() {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.category_id || !form.unit_id) {
      setError("Name, Category aur Unit required hain!");
      return;
    }
    setSaving(true);
    setError("");
  ...
  const handleLogSubmit = async () => {
    if (!logForm.item_id || !logForm.quantity_used || !logForm.department) {
      setError("Item, Quantity aur Department required hain!");
      return;
    }
    setSaving(true);
    setError("");
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {items.map((item) => (
                <tr
                  key={item.id}
                  className={`border-t border-gray-100 ${item.is_active === false ? "opacity-60" : ""}`}
                >
