        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{ height: photos.length ? 260 : 100 }}
            >
              {photos.length > 0 ? (
                <>
                  <Image
              ...
              {photos.length > 0 && (
                <div className="absolute bottom-5 left-4">
                  <p className="text-white text-2xl font-bold drop-shadow-sm">
                    Room {room.room_number}
                  </p>
                  {typeCfg && (
                    <p className={clsx("text-sm font-medium", typeCfg.color)}>
                      {typeCfg.icon} {room.room_type}
                    </p>
                  )}
                </div>
              )}
            ...
            >
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  {
                    label: "Rate",
                    value: `PKR ${Number(room.price_per_night).toFixed(0)}/night`,
                  },
                  { label: "Floor", value: `Floor ${room.floor}` },
                  { label: "Guests", value: `${room.capacity} guests` },
                  {
                    label: "Size",
                    value: room.size_sqft ? `${room.size_sqft} sqft` : "—",
                  },
                ].map(({ label, value }) => (
                  <div
                    key={label}
                    className="bg-muted/50 border border-border rounded-xl px-3 py-2.5 text-center"
                  >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {amenities.map((a) => (
                      <span
                        key={a}
                        className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-muted border border-border text-xs font-medium text-foreground"
                      >
                        <CheckCircle2 size={9} className="text-gold shrink-0" />{" "}
                        {a}
                      </span>
                    ))}
                  </div>
