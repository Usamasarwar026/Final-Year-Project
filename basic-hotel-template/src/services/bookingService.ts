...
export async function createBooking(
  ...
  await createNotification({
    ...
    message:
      "A new booking for Room " + room.room_number + " has been received.",
    type: "booking",
    priority: "High",
    module: "booking",
    referenceId: String(row.booking_id),
...
export async function deleteBooking(id: number): Promise<boolean> {
  try {
    ...
    if (
      ["Pending", "Confirmed", "CheckedIn"].includes(booking.status as string)
    ) {
      await prisma.room.update({
        where: { room_id: booking.room_id },
        data: { status: "Available" as any },
...
export async function onBookingCheckout(bookingId: number): Promise<void> {
  try {
    ...
    console.log(
      "[Housekeeping] Auto-task created for room " +
        booking.room.room_number +
        " after checkout",
    );
  } catch (err) {
