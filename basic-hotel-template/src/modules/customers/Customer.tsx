...
function ProfileDrawer({
  ...
  const handleToggleSuspend = async () => {
    if (!profile) return;
    const res = await onUpdate(customerId.toString(), {
      is_active: !profile.is_active,
    });
    if (res.ok) {
      refresh();
      toast.success(
        profile.is_active ? "Customer deactivated" : "Customer activated",
      );
...
export default function Customers() {
  ...
  const handleStatus = useCallback((val: "all" | "active" | "inactive") => {
    setStatusFilter(val);
    setPage(1);
  }, []);
